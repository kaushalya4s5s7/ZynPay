...
const PaymentStatus = ({
    ...
    // For ethers.js implementation (chain 1001)
    ...
    // Check transaction status when using ethers and chain 1001
    useEffect(() => {
        if (chainId === 1001 && txHash) {
    ...
    // Similarly for approval transaction when using chain 1001
    useEffect(() => {
        if (chainId === 1001 && approvalTxHash) {
    ...
    useEffect(() => {
        const isSuccess = chainId === 1001 ? ethersTxStatus.isSuccess : isTxSuccess;
        const isError = chainId === 1001 ? ethersTxStatus.isError : isTxError;
    ...
    const isProcessing = chainId === 1001
    ...
    const displayError = chainId === 1001 ? ethersTxStatus.error : txError;
    const displayIsError = chainId === 1001 ? ethersTxStatus.isError : isTxError;
    const displayIsSuccess = chainId === 1001 ? ethersTxStatus.isSuccess : isTxSuccess;
