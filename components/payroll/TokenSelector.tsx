...
const TokenSelector = ({
    ...
    // Check if we're on kaia chain
    const iskaiaChain = chainId === kaia_CHAIN_ID;

    // Only enable wagmi hook if NOT on kaia chain
    const {
        data: balance,
        ...
        chainId: chainId,
        query: {
            // Disable the hook for kaia chain
            enabled: isConnected && !!selectedToken && !!address && !iskaiaChain,
            retry: 3,
            retryDelay: 1000
        }
    });
    ...
    // For kaia chain, fetch directly with ethers whenever relevant dependencies change
    useEffect(() => {
        if (iskaiaChain && isConnected && address && selectedToken && !fallbackBalance && !isFallbackLoading) {
            ...
            fetchBalanceWithEthers()
                ...
                .catch(err => {
                    ...
                    console.error("Error fetching kaia chain balance:", err);
                })
                .finally(() => {
                    setIsFallbackLoading(false);
                });
        }
    }, [iskaiaChain, isConnected, address, selectedToken, fetchBalanceWithEthers, fallbackBalance, isFallbackLoading]);

    // Effect for non-kaia chains when wagmi hook fails
    useEffect(() => {
        if (!iskaiaChain && balanceError && !fallbackBalance && !isFallbackLoading) {
            setIsFallbackLoading(true);
            setFallbackError(null);
    ...
    }, [iskaiaChain, balanceError, fetchBalanceWithEthers, fallbackBalance, isFallbackLoading]);
    ...
    const handleRefreshBalance = () => {
        ...
        if (iskaiaChain) {
            // For kaia, directly trigger ethers fetch by resetting state
            setIsFallbackLoading(true);
            fetchBalanceWithEthers()
                .then(result => {
                    if (result) setFallbackBalance(result);
    ...
    // Effect to fetch balance when token/chain changes (for non-kaia chains)
    useEffect(() => {
        if (isConnected && selectedToken && address && !iskaiaChain) {
            refetchBalance();
        }
    }, [selectedToken?.address, refetchBalance, isConnected, address, iskaiaChain]);

    // Determine loading state based on active fetch method
    const isAnyBalanceLoading = (iskaiaChain ? isFallbackLoading : (isBalanceLoading || isFallbackLoading));

    // Error state - for kaia we only check fallback error, for others we check both
    const hasRealBalanceError = iskaiaChain ? fallbackError : (balanceError && fallbackError);
